...
class CoverageTargetReporter extends ReportBase {
  constructor(opts = {}) {
    super();
    this.target = typeof opts.target === 'number' ? opts.target : DEFAULT_COVERAGE_TARGET_PCT;
  }
  ...
  onStart(node, context) {
    ...
    cw.println('');
    cw.println('Target: ' + this.target + '% (see coverage.thresholds in vite.config.ts; uncomment to enforce)');
    for (const key of METRICS) {
      ...
      const pad = label.length < 10 ? label + ' '.repeat(10 - label.length) : label;
      const line = '  ' + pad + ': ' + m.pct + '% ( ' + m.covered + '/' + m.total + ' ) out of ' + this.target + '%' + (more > 0 ? ' — need ' + more + ' more' : '');
      cw.println(cw.colorize ? cw.colorize(line, context.classForPercent(key, m.pct)) : line);
    }
    cw.close();
  }
}
