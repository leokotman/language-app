...
test.describe("Study (session flow)", () => {
   ...
   */
  test("start session, do one card (flashcard only), rate, see session complete", async ({
    page,
  }) => {
    ...
    await page.getByRole("button", { name: "Add" }).click();
    await expect(
      page.getByText("e2e-study-word — e2e-study-translation").first(),
    ...
    const noCardsDue = page.getByText(
      "No cards due today for this language pair",
    );
    if (await noCardsDue.isVisible()) {
      test.skip(
        true,
        "No cards due for selected pair; seed data or add word in Library first",
      );
    }
    ...
    await expect(
      page.getByRole("checkbox", { name: /Flashcard \(word → translation/ }),
    ).toBeChecked();
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const revealBtn = card.getByRole("button", {
        name: /Reveal (translation|word)/,
      });
      const checkBtn = card.getByRole("button", { name: "Check" });
      const goodBtn = card.getByRole("button", { name: "Good" });

      const sawReveal = await revealBtn
      ...
      if (sawReveal) {
        ...
        await checkBtn.isVisible({ timeout: 500 }).catch(() => false)
      ) {
        await page.getByLabel("Translation").last().fill("x");
        await checkBtn.click();
        await goodBtn.click();
      } else if (
        await goodBtn.isVisible({ timeout: 3000 }).catch(() => false)
      ) {
        await goodBtn.click();
      } else {
        ...
        if (await recordBtn.isVisible({ timeout: 500 }).catch(() => false)) {
          ...
          await stopBtn.isVisible({ timeout: 500 }).catch(() => false)
        ) {
          await stopBtn.click();
        } else {
          const mcOption = card
            ...
            .filter({
              hasNotText:
                /^(Again|Hard|Good|Easy|Check|Reveal|Play word|Record|Stop|Play back)/,
            })
            .first();
      ...
      await page
        .getByTestId("study-session-complete")
        .or(page.getByTestId("study-card"))
        .waitFor({ state: "visible", timeout: 10000 })
        .catch(() => {});
    }

    await expect(page.getByTestId("study-session-complete")).toBeVisible({
      timeout: 5000,
    });
    await expect(
    ...
    await expect(
      page.getByRole("button", { name: "Back to study" }),
    ).toBeVisible();
  });
});
