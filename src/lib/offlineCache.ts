...
export type UserVocabularyListItem = UserVocabularyRow & { vocabulary: VocabularyRow | null }
...
export async function mergeAppVocabulary(newRows: VocabularyRow[]): Promise<void> {
  ...
  offlineLog('mergeAppVocabulary', { added: newRows.length, totalAfter: merged.length })
}

export async function getUserLanguages(userId: string): Promise<UserLanguageRow[]> {
  const cached = await get<UserLanguageRow[]>(KEY_USER_LANGUAGES_PREFIX + userId)
  return cached ?? []
}

export async function setUserLanguages(userId: string, rows: UserLanguageRow[]): Promise<void> {
  await set(KEY_USER_LANGUAGES_PREFIX + userId, rows)
}

export async function getUserVocabularyList(userId: string): Promise<UserVocabularyListItem[]> {
  const cached = await get<UserVocabularyListItem[]>(KEY_USER_VOCABULARY_LIST_PREFIX + userId)
  return cached ?? []
}
...
export async function getDictionaryLookupCache(
  ...
  const raw = await get<{ entries: DictionaryLookupEntry[] }>(KEY_LOOKUP_PREFIX + queryKey)
...
export async function setDictionaryLookupCache(
  ...
  const updated = [queryKey, ...keys.filter((k) => k !== queryKey)].slice(0, LOOKUP_CACHE_MAX_ENTRIES)
  ...
  offlineLog('setDictionaryLookupCache', { queryKey: queryKey.slice(0, 30), entriesCount: entries.length })
}
