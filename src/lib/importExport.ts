...
function validateRow(
  ...
  const language_from =
    typeof record.language_from === "string"
      ? record.language_from.trim().toLowerCase()
      : "";
  const language_to =
    typeof record.language_to === "string"
      ? record.language_to.trim().toLowerCase()
      : "";

  if (!word)
    return { ok: false, error: `Row ${index + 1}: missing or empty word` };
  if (!translation)
    return {
      ok: false,
      error: `Row ${index + 1}: missing or empty translation`,
    };
  if (
    !SUPPORTED_LANGS.includes(language_from as (typeof SUPPORTED_LANGS)[number])
  ) {
    return {
      ok: false,
      error: `Row ${index + 1}: language_from must be en, ru, or sr`,
    };
  }
  if (
    !SUPPORTED_LANGS.includes(language_to as (typeof SUPPORTED_LANGS)[number])
  ) {
    return {
      ok: false,
      error: `Row ${index + 1}: language_to must be en, ru, or sr`,
    };
  }
  if (language_from === language_to) {
    return {
      ok: false,
      error: `Row ${index + 1}: language_from and language_to must differ`,
    };
  }
...
function parseCsvText(text: string): ParseResult {
  ...
  if (!header.includes("word") || !header.includes("translation")) {
    return {
      rows: [],
      errors: [
        "CSV must have header: word,translation,language_from,language_to",
      ],
    };
  }
