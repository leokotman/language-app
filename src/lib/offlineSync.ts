 ...
 */
export async function syncForOffline(userId: string): Promise<SyncForOfflineResult> {
  ...
  try {
    ...
    if (appError) {
      ...
      return { success: false, error: appError instanceof Error ? appError : new Error(String(appError)) }
    }
    ...
    if (langsError) {
      ...
      return { success: false, error: langsError instanceof Error ? langsError : new Error(String(langsError)) }
    }

    const { error: libError } = await listUserVocabulary(userId)
    if (libError) {
      logError('offlineSync.listUserVocabulary', libError)
      return { success: false, error: libError instanceof Error ? libError : new Error(String(libError)) }
    }

    offlineLog('syncForOffline done', { success: true, appVocabCount: appVocab.length })
    return { success: true }
  } catch (err) {
