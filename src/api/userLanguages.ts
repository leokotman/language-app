...
import {
  getUserLanguages as getCachedUserLanguages,
  setUserLanguages,
...
export async function getUserLanguages(userId: string): Promise<{
  ...
  if (isOffline) {
    ...
    offlineLog("getUserLanguages offline", {
      userId,
...
export async function removeUserLanguage(
  id: string,
...
export async function addBidirectionalPair(
  ...
  const rows: UserLanguageInsert[] = [
    {
      user_id: userId,
      native_code: languageCodeA,
      learning_code: languageCodeB,
    },
    {
      user_id: userId,
      native_code: languageCodeB,
      learning_code: languageCodeA,
    },
  ];
  const { data, error } = await supabase
    .from("user_languages")
