...
export function getLanguagePairLabel(
  languageFrom: string,
  languageTo: string,
): string {
  const pair = SUPPORTED_LANGUAGE_PAIRS.find(
    (option) =>
      option.native === languageFrom && option.learning === languageTo,
  );
...
export async function processLibraryImport(
  ...
  for (const row of rows) {
    ...
    try {
      ...
      throw new Error(
        `Import stopped after ${result.added} words. Could not add "${row.word}".`,
      );
    }
  }
...
export function buildLibraryImportMessage(
  result: ProcessLibraryImportResult,
): string {
  ...
  return [
    {
      ...
      label: getLanguagePairLabel(sourceLang, targetLang),
    },
    {
      value: `${targetLang}-${sourceLang}`,
      label: getLanguagePairLabel(targetLang, sourceLang),
    },
  ];
}
