...
describe("getAuthErrorMessage", () => {
  ...
  it("returns fallback when message is not a string", () => {
    expect(getAuthErrorMessage({ message: 123 })).toBe(
      "An error occurred. Please try again.",
    );
  });
});
...
describe("isSupabaseTableMissingError", () => {
  it('returns true for "does not exist" or "relation" message', () => {
    expect(
      isSupabaseTableMissingError({
        message: 'relation "user_languages" does not exist',
      }),
    ).toBe(true);
    expect(
      isSupabaseTableMissingError({ message: "relation xyz missing" }),
    ).toBe(true);
  });

  it("returns true for 42P01 or PGRST301 code", () => {
    expect(isSupabaseTableMissingError({ message: "x", code: "42P01" })).toBe(
      true,
    );
    expect(
      isSupabaseTableMissingError({ message: "x", code: "PGRST301" }),
    ).toBe(true);
  });

  it("returns false for generic error or non-string message", () => {
    expect(isSupabaseTableMissingError({ message: "Network failure" })).toBe(
      false,
    );
