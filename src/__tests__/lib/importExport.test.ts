...
function createFile(content: string, name: string, _type: string): File {
...
describe('importExport', () => {
  describe('exportToCsv', () => {
    it('exports empty array as header only', () => {
      expect(exportToCsv([])).toBe('word,translation,language_from,language_to')
    })
    it('exports rows with escaped fields', () => {
      const rows: LibraryExportRow[] = [
        { word: 'hello', translation: 'привет', language_from: 'en', language_to: 'ru' },
      ]
    ...
    it('escapes double quotes and commas in fields', () => {
      const rows: LibraryExportRow[] = [
        { word: 'say "hi"', translation: 'a,b', language_from: 'en', language_to: 'ru' },
      ]
  ...
  describe('parseLibraryFile', () => {
    it('parses JSON file and returns rows', async () => {
      const json = JSON.stringify([
        { word: 'hello', translation: 'hi', language_from: 'en', language_to: 'ru' },
      ])
    ...
    it('parses CSV file with header', async () => {
      const csv = 'word,translation,language_from,language_to\nhello,привет,en,ru'
    ...
    it('validates row fields and returns errors for invalid rows', async () => {
      const json = JSON.stringify([
        { word: 'ok', translation: 'ok', language_from: 'en', language_to: 'ru' },
    ...
    it('rejects unsupported language codes', async () => {
      ...
      const result = await parseLibraryFile(file)
      expect(result.rows).toHaveLength(0)
      expect(result.errors.some((e) => e.includes('language_from must be'))).toBe(true)
    })
