...
describe('vocabulary API', () => {
  ...
  describe('listAllAppVocabulary', () => {
    it('returns app vocabulary and no error', async () => {
      ...
      vi.mocked(supabase.from).mockReturnValue(createSupabaseChain({ data, error: null }) as never)
  ...
  describe('listVocabulary', () => {
    ...
    it('returns cached app vocabulary when offline', async () => {
      ...
      const result = await listVocabulary({ languageFrom: 'en', languageTo: 'ru' })
    ...
    it('returns cached filtered data on network error', async () => {
      const cached = [mockVocabularyRow]
      vi.mocked(supabase.from).mockReturnValue(
        createSupabaseChain({ data: null, error: new Error('Failed to fetch') }) as never
      )
  ...
  describe('updateVocabulary', () => {
    it('updates and returns row', async () => {
      ...
      const result = await updateVocabulary('v-1', { translation: 'здравствуй' })
  ...
  describe('deleteVocabulary', () => {
    it('deletes by id and returns no error', async () => {
      vi.mocked(supabase.from).mockReturnValue(createSupabaseChain({ error: null }) as never)
  ...
  describe('listDueToday', () => {
    ...
    it('filters by pairKey when provided', async () => {
      const enRu = { ...dueRow, vocabulary: { ...mockVocabularyRow, language_from: 'en', language_to: 'ru' } }
      const ruEn = { ...dueRow, id: 'uv-2', vocabulary: { ...mockVocabularyRow, language_from: 'ru', language_to: 'en' } }
      const other = { ...dueRow, id: 'uv-3', vocabulary: { ...mockVocabularyRow, language_from: 'sr', language_to: 'en' } }
      vi.mocked(supabase.from).mockReturnValue(
        createSupabaseChain({ data: [enRu, ruEn, other], error: null }) as never
      )
    ...
    it('filters by languageFrom and languageTo when provided', async () => {
      const matching = { ...dueRow, vocabulary: { ...mockVocabularyRow, language_from: 'en', language_to: 'ru' } }
      vi.mocked(supabase.from).mockReturnValue(
  ...
  describe('listUserVocabulary', () => {
    const listItem = { ...mockUserVocabularyRow, vocabulary: mockVocabularyRow }
    ...
    it('returns data from Supabase and writes cache when online', async () => {
      ...
      expect(result.data).toEqual(data)
      expect(result.error).toBeNull()
      expect(offlineCache.setUserVocabularyList).toHaveBeenCalledWith('user-1', data)
    })
  ...
  describe('addToUserLibrary', () => {
    it('inserts and returns user_vocabulary row', async () => {
      ...
      vi.mocked(supabase.from).mockReturnValue(
        createSupabaseChain({ data: mockUserVocabularyRow, error: null }) as never
      )
  ...
  describe('updateUserVocabulary', () => {
    it('updates FSRS fields and returns row', async () => {
      const updates: UserVocabularyUpdate = { due: '2026-02-19T12:00:00.000Z', state: 1 }
  ...
  describe('addWordToLibrary', () => {
    it('creates vocabulary then user_vocabulary and returns combined', async () => {
      const vocabRow = { ...mockVocabularyRow, source: 'user' as const, created_by: 'user-1' }
      ...
      vi.mocked(supabase.from).mockImplementation(() => {
        ...
        return createSupabaseChain({ data: { ...mockUserVocabularyRow, vocabulary: vocabRow }, error: null }) as never
      })
    ...
    it('returns error when createVocabulary fails', async () => {
      vi.mocked(supabase.from).mockReturnValue(
        createSupabaseChain({ data: null, error: new Error('create failed') }) as never
      )
      const result = await addWordToLibrary('user-1', {
        word: 'hi',
        translation: 'привет',
