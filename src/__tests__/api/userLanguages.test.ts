...
describe('userLanguages API', () => {
  ...
  describe('getUserLanguages', () => {
    it('returns data from Supabase and writes cache when online', async () => {
      ...
      expect(offlineCache.setUserLanguages).toHaveBeenCalledWith('user-1', data)
    })

    it('returns error when Supabase returns error and not network error', async () => {
      const err = new Error('db error')
      vi.mocked(supabase.from).mockReturnValue(createSupabaseChain({ data: null, error: err }) as never)
      const result = await getUserLanguages('user-1')
  ...
  describe('addUserLanguage', () => {
    it('inserts and returns new row', async () => {
      ...
      vi.mocked(supabase.from).mockReturnValue(
        createSupabaseChain({ data: mockUserLanguageRow, error: null }) as never
      )
  ...
  describe('addBidirectionalPair', () => {
    ...
    it('inserts both directions and returns rows', async () => {
      const rows = [
        { ...mockUserLanguageRow, id: 'ul-1', native_code: 'en', learning_code: 'ru' },
        { ...mockUserLanguageRow, id: 'ul-2', native_code: 'ru', learning_code: 'en' },
      ]
      vi.mocked(supabase.from).mockReturnValue(createSupabaseChain({ data: rows, error: null }) as never)
