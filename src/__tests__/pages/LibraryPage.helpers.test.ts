...
describe("LibraryPage.helpers", () => {
  ...
  describe("buildLibraryImportMessage", () => {
    it("returns failure message when only parse errors", () => {
      ...
      expect(buildLibraryImportMessage(result)).toBe(
        "Import failed: Row 1: bad",
      );
    });
    ...
    it("returns success message when no errors", () => {
      expect(
        buildLibraryImportMessage({ added: 3, skipped: 0, parseErrors: [] }),
      ).toBe("Imported 3 words. ");
    });
    it("includes skipped count when skipped > 0", () => {
      expect(
        buildLibraryImportMessage({ added: 1, skipped: 1, parseErrors: [] }),
      ).toBe("Imported 1 words. Skipped 1 duplicates.");
    });
  });

  describe("downloadBlob", () => {
    it("creates blob, object URL, and triggers download link", () => {
      const clickFn = vi.fn();
      const revokeSpy = vi
      ...
      downloadBlob("content", "text/plain", "export.txt");
