...
**Git:** Branch names must include project code and task number so the commit-msg hook adds scope (e.g. `feat/lang-005-expand-seed`). One logical change per commit; message describes that single change (no “X and Y” in subject).
