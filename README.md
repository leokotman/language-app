...
5. Run E2E tests: `npm run test:e2e` (starts dev server automatically).  
   For authenticated Library flows (add/edit/delete word), set `E2E_TEST_EMAIL` and `E2E_TEST_PASSWORD` in `.env` (see `.env.example`). Without them, only unauthenticated and home tests run.

## Tech

React 18+, TypeScript, Vite, MUI, React Router, Supabase, TanStack Query, Zustand, vite-plugin-pwa, Vitest, Playwright.
